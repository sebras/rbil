...
#define divider_line(line) (strncmp(line,"--------",8) == 0)
...
Options:\n\
\t-b\tboldface title lines and section headings\n\
\t-B\tboldface using printer control codes instead of overprinting\n\
\t-d\t(duplex) print even/odd pages with different indents\n\
\t-e\tassume 'elite' mode (96 chars per line) for default printer\n\
\t-ffile\twrite all data structure formats to 'file'\n\
\t-Ffile\tprint only entries matching filtering info in 'file'\n\
\t-H\tadd page headers showing interrupt call(s) on page\n\
\t-iN\tindent output N spaces (overridden by some printers)\n\
\t-I\tprinter supports IBM graphics characters\n\
\t-k\tkeep original divider lines\n\
\t-lN\tprint N lines per page (default varies by prntr, 0=infinite)\n\
\t-LN\tassume N lines on a page, use linefeeds if > #lines printed\n\
\t-m\tlist is in multiple parts starting with the named file\n\
\t-nN\tassume N pages have been printed from previous parts\n\
\t-p\tadd page numbers\n\
\t-Pname\tassume printer 'name' (-P? lists supported printers)\n\
\t-rN:M\tprint only pages N through M\n\
\t-sfile\twrite a one-line-per-function summary to 'file'\n\
\t-tN\tselect typeface N for the chosen printer\n\
\t-wN\tscan N lines from bottom of page for good place to break\n\
...
char *line ;
{
   return (strncmp(line,"Format of ",10) == 0 ||
	   ...
	   strcmp(strchr(line,'\0')-12,"called with:") == 0
          ) ;
}
...
char *argv[] ;
{
   ...
   if (lines_per_page > MAXPAGE)
      {
      fputs("Surely you jest!  I can't handle pages that long.\n\n", stderr) ;
      usage() ;
      }
	 ...
	 {
	 fputs("Surely your printer can handle at least 20 lines per page!\n",stderr) ;
	 fputs("Adjusting page length....\n",stderr) ;
	 lines_per_page = 20 ;
	 }
